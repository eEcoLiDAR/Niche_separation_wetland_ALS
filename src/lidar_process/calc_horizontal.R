...
for (i in filelist) {
  ...
  height_class=reclassify(dsm, c(-Inf,4,1, 4,Inf,0))
  ...
  writeRaster(sd_dsm_5,paste(name,"_dsm_sd_20m.tif",sep=""),overwrite=TRUE)
  writeRaster(med_dsm_5,paste(name,"_dsm_med_20m.tif",sep=""),overwrite=TRUE)
  ...
  writeRaster(lowveg,paste(name,"_lowveg_count_50m.tif",sep=""),overwrite=TRUE)
