...
for (i in filelist) {
  ...
  prop_trees=focal(height_class2,w=matrix(1,11,11), fun=sum, pad=TRUE,na.rm = TRUE)
  ...
  writeRaster(sd_dsm_11,paste(name,"_dsm_sd_50m_masked.tif",sep=""),overwrite=TRUE)
  writeRaster(sd_lowveg_11,paste(name,"_lowveg_sd_50m_masked.tif",sep=""),overwrite=TRUE)
  
  writeRaster(prop_lowveg,paste(name,"_prop_lowveg_50m_masked.tif",sep=""),overwrite=TRUE)
  writeRaster(prop_trees,paste(name,"_prop_trees_50m_masked.tif",sep=""),overwrite=TRUE)
