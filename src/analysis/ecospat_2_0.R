...
workingdirectory="D:/Koma/_PhD/Chapter3/Data_Preprocess/escience_lidar_data_v2/selected_layers_for_chapter3/masked/100m/"
...
fviz_contrib(res1.pca, choice = "var", axes = 1, top = 5)

res2.pca=PCA(data_merged[,c(4,6,10,11)], scale.unit = TRUE, ncp = 5, graph = FALSE)

fviz_pca_var(res2.pca, col.var = "contrib",
             gradient.cols = c("#00AFBB", "#E7B800", "#FC4E07")
)
fviz_contrib(res2.pca, choice = "var", axes = 1, top = 5)

res3.pca=PCA(data_merged[,c(7,8,9,10)], scale.unit = TRUE, ncp = 5, graph = FALSE)

fviz_pca_var(res3.pca, col.var = "contrib",
             gradient.cols = c("#00AFBB", "#E7B800", "#FC4E07")
)
fviz_contrib(res3.pca, choice = "var", axes = 1, top = 5)
...
pca.env<-dudi.pca(data_merged[,1:12],scannf=FALSE,center=TRUE,nf=2)
par(mfrow=c(1,1))
